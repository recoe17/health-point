...
export default function MetricCard({ label, value, onClick, accentColor = "text-red-400" }: MetricCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full text-left rounded-xl border border-slate-700/60 bg-slate-900/40 px-4 py-3 hover:bg-slate-800/60 hover:border-red-500/60 transition flex items-center justify-between group"
    >
